/-
  KuramotoFirstMomentConcreteV9.lean
  ===================================
  Drops `hα_neg` from V8 by replacing it with the weaker
    hα_bdd : ∀ ω t, 0 ≤ α ω t ∧ α ω t ≤ 1
  (global trajectory bounds, no extension convention required).

  Vs V8: `hα_neg` (extension to t<0 by constant value) is replaced
  by `hα_bdd` (global [0,1]-invariance of the trajectory). Neither
  implies the other; hα_bdd is the physically natural invariant.

  Derivation of hα_sq_int: for any t,
    0 ≤ α ω t ≤ 1  (from hα_bdd)
    0 < α_star ω < 1  (from explicitEquil bounds)
    → -1 < α ω t - α_star ω < 1
    → (α ω t - α_star ω)² < 1 → Integrable via integrable_const 1.
  No case split on sign of t needed.
...
theorem kuramoto_first_moment_concrete_v9 [IsProbabilityMeasure μ]
    (γ : Ω → ℝ) (K : ℝ)
