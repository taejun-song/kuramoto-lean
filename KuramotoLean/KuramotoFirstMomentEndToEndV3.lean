/-
  KuramotoFirstMomentEndToEndV3.lean
  ===================================
  End-to-end convergence combining:
  - sc_fixed_point_exists_continuum (exp 304): ∃ r_star from K·∫1/γ > 2
  - kuramoto_first_moment_concrete_v9 (exp 308): drops hα_neg via hα_bdd

  Vs end-to-end V2 (exp 307): replaces
    hα_neg : ∀ ω t, t ≤ 0 → α ω t = α ω 0
  with the weaker physical condition
    hα_bdd : ∀ ω t, 0 ≤ α ω t ∧ α ω t ≤ 1
  (global [0,1]-invariance; no extension convention required).

  Net signature: no explicit r_star, no hα_sq_int, no hα_neg.
  hα_bdd replaces hα_neg — a strictly more natural hypothesis.
...
theorem kuramoto_first_moment_end_to_end_v3 [IsProbabilityMeasure μ]
    (γ : Ω → ℝ) (K : ℝ)
  ...
  exact ⟨r_star, hr_pos, hr_lt,
    kuramoto_first_moment_concrete_v9 γ K hK hγ_pos hγ_level hγ_int
      r_star hr_pos hr_sc r α hα_ode h_sc hα_int hα_bdd hα_inv
