/-
  ContinuumSolvedWired7.lean
  ==========================
  Weakens `hδ₀_body_lb` from "for all M > 0" to "eventually for M ≥ M₀".

  KEY CHANGE from wired6: the structural hypothesis
    hδ₀_body_lb : ∃ c > 0, ∀ M > 0, c/M ≤ δ₀_body M
  is weakened to
    hδ₀_body_lb : ∃ c > 0, ∃ M₀ > 0, ∀ M ≥ M₀, c/M ≤ δ₀_body M

  This is sufficient because wired6's proof only uses hc_bound inside a
  filter_upwards for large M (via eventually_ge_atTop). The bound for small M
  is never needed.

  MOTIVATION: enables the standard Kuramoto model (γ(ω) = |ω|, γ_min = 0)
  where δ₀_body M ≈ K·r₀/(2M) satisfies c/M ≤ δ₀_body M only for M ≥ M₀.
...
theorem kuramoto_continuum_wired7 [IsProbabilityMeasure μ]
    (γ : Ω → ℝ) (K : ℝ)
    (hK : 0 < K) (hγ : ∀ ω, 0 ≤ γ ω)
    ...
    (hδ₀_body_lb : ∃ c : ℝ, 0 < c ∧ ∃ M₀ : ℝ, 0 < M₀ ∧ ∀ M, M₀ ≤ M → c / M ≤ δ₀_body M) :
    Tendsto r atTop (nhds r_star) := by
  obtain ⟨c, hc_pos, M₀, hM₀_pos, hc_bound⟩ := hδ₀_body_lb
  ...
  have hCM_vanish : Tendsto CM atTop (nhds 0) := by
    ...
    · filter_upwards [eventually_ge_atTop (max (max (K * r_min) (K * r_star)) M₀),
                      h_τ_vanish.eventually (gt_mem_nhds (show (0:ℝ) < 1/2 by norm_num)),
                      eventually_gt_atTop (0:ℝ)] with M hM hτ hM_pos
      have hM_Kr : K * r_min ≤ M := le_trans (le_max_left _ _) (le_trans (le_max_left _ _) hM)
      have hM_Ks : K * r_star ≤ M := le_trans (le_max_right _ _) (le_trans (le_max_left _ _) hM)
      have hM_M₀ : M₀ ≤ M := le_trans (le_max_right _ _) hM
